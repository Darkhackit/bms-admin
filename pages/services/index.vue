...
const canEditPermission = computed(() => can('show', 'service') || can('manage', 'all'));
const canDeletePermission = computed(() => can('delete', 'service') || can('manage', 'all'));
const editForm = ref({
  id:'',
  name:'',
  description:''
...
const updateData = async () => {
  processing.value = true
  try {
    await api.patch(`/services/${editForm.value.id}`, editForm.value)
    await getData()
    processing.value = false
      ...
      <div v-else>
        <div>
          <div class="flex justify-between items-center">
            <div>
              <h1 class="uppercase font-medium">Services</h1>
            </div>
            <div v-if="$can('manage','all') || $can('add','service')">
              <UButton @click.prevent="isOpen = true" color="black" variant="solid">Add Service</UButton>
            </div>
          </div>
          <div>
            <div class="flex space-x-2 px-3 py-3.5 border-b border-gray-200 dark:border-gray-700">
              <UInput v-model="q" clearable  placeholder="Filter service..." />
          ...
          <form @submit.prevent="addData">
            <div class="space-y-4">
              ...
              <div class="space-y-2">
                ...
                <UTextarea v-model="form.description" @input="clearErrors('description')" :class="errors.description ? 'border border-red-500' :''"  placeholder="Description"  />
                <small class="text-red-500" v-if="errors?.description">{{errors.description}}</small>
              </div>
              <div class="flex justify-end" v-if="$can('manage','all') || $can('add','service')">
                <UButton type="submit" :loading="processing" color="black" variant="solid">Add Service</UButton>
              </div>
            </div>
          </form>
        </div>
        ...
        <div class="" >
          <form @submit.prevent="updateData">
            <div class="space-y-4">
              ...
              <div class="space-y-2">
                <label>Description</label>
                <UTextarea v-model="editForm.description" @input="clearError('description')" :class="error.description ? 'border border-red-500' :''"  placeholder="Description"  />
                <small class="text-red-500" v-if="error?.description">{{error.description}}</small>
              </div>
              <div class="flex justify-end" v-if="$can('manage','all') || $can('update','service')">
                <UButton type="submit" :loading="processing" color="black" variant="solid">Update Service</UButton>
              </div>
            </div>
          </form>
        </div>
