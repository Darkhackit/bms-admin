...
const canEditPermission = computed(() => can('show', 'template') || can('manage', 'all'));
const canDeletePermission = computed(() => can('delete', 'template') || can('manage', 'all'));
const CanApprovalTemplate = computed(() => can('approve', 'template') || can('manage', 'all'));
...
const updateData = async () => {
  ...
  try {
    await api.patch(`/templates/${editForm.value.id}`, editForm.value)
    await getData()
    processing.value = false
...
const getSingleData = async (id,user_id) => {
  processing.value = true
  try {
    let response = await api.get(`/templates/${id}/${user_id}`)
    console.log(response)
    editForm.value.id = response.data?.id
    editForm.value.title = response.data?.title
    editForm.value.message = response.data?.message
...
const getData = async () => {
  ...
  processing.value = true
  try{
    let response = await api.get("/templates",{params})
...
const deleteData = async (id,user_id) => {
  if (confirm('Are you sure?')) {
    processing.value = true
    try {
      await api.delete(`/templates/${id}/${user_id}`)
      await getData()
      processing.value = false
      toast.add({
...
const updateStatus = async (id) => {
  if (confirm('Are you sure?')) {
    processing.value = true
    try {
      await api.patch(`/templates/update/${bForm.value.id}`,bForm.value)
      await getData()
      processing.value = false
             ...
             <div class="flex space-x-2 px-3 py-3.5 border-b border-gray-200 dark:border-gray-700">
              ...
              <div>
                <div v-if="$can('manage','all') || $can('add','group')">
                  <UButton @click.prevent="isOpen = true" color="black" variant="solid">Add Template</UButton>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="" >
          <form @submit.prevent="addData">
            <div class="space-y-2">
              <div class="space-y-2">
                <label>Title</label>
                <UInput v-model="form.title" @input="clearErrors('title')" :input-class="errors.title ? 'border border-red-500' :''"  placeholder="Title"  />
              ...
              <div>
                <label>Message</label>
                <UTextarea v-model="form.message"  size="xl" :rows="6"/>
        ...
        <div class="" >
          <form @submit.prevent="updateData">
            <div class="space-y-2">
              <div class="space-y-2">
                <label>Title</label>
                <UInput v-model="editForm.title" @input="clearError('title')" :input-class="error.title ? 'border border-red-500' :''"  placeholder="Title"  />
                <small class="text-red-500" v-if="error?.title">{{error.title}}</small>
              </div>
              <div class="space-y-2">
                <label>Service</label>
                <USelectMenu
              ...
              <div>
                <label>Message</label>
                <UTextarea v-model="editForm.message"  size="xl" :rows="6"/>
                <small class="text-red-500" v-if="error?.message">{{error.message}}</small>
              </div>
              <div class="flex justify-end" v-if="$can('manage','all') || $can('add','group')">
                <UButton type="submit" :loading="processing" color="black" variant="solid">Update Template</UButton>
              </div>
            </div>
          </form>
        </div>
